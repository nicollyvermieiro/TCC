...
<head>
    ...
    <style>
        ...
        .card-header { background-color: #0d6efd; color: #fff; font-weight: bold; font-size: 1.2rem; text-align: center; border-top-left-radius: 12px; border-top-right-radius: 12px; }
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form method="post" action="?route=usuarios/atualizar">
                ...
                <input type="hidden" name="cargo_nome" id="cargo_nome" value="<?= htmlspecialchars($dados['cargo_nome'] ?? '') ?>">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="cargo_id" name="cargo_id" required>
                        ...
                        <?php foreach($cargos as $cargo): ?>
                            <option value="<?= htmlspecialchars($cargo['id']) ?>" <?= ($dados['cargo_id'] ?? null) == $cargo['id'] ? 'selected' : '' ?>>
                ...
                <div class="mb-3" id="setor_div" style="display:none;">
                    ...
                    <select class="form-select" id="setor_id" name="setor_id">
                        ...
                        <?php foreach($setores as $setor): ?>
                            <option value="<?= htmlspecialchars($setor['id']) ?>" <?= ($dados['setor_id'] ?? null) == $setor['id'] ? 'selected' : '' ?>>
...
const cargoNomeInput = document.getElementById('cargo_nome');

function atualizarSetor() {
    const selectedOption = cargoSelect.options[cargoSelect.selectedIndex];
    const tipo = selectedOption.text.toLowerCase();
    cargoNomeInput.value = selectedOption.text;
